...
This profound mathematical identity fundamentally arises from the spatial inversion symmetry and reciprocity of the electromagnetic translation operator.
This means that the adjoint translation can be evaluated by: (i) applying $\mathbf{S}$, (ii) performing the same forward translation $\mathbf{A}$, (iii) applying $\mathbf{S}$ again, and (iv) complex-conjugating.
...
\begin{equation}
  \frac{\mathrm{Re}(\bm{r}^\top \bm{r})}{\|\bm{b}\|^2} < \varepsilon
  \label{eq:convergence}
\end{equation}
where $\bm{r}$ is the bilinear (not sesquilinear) residual and $\varepsilon = 10^{-6}$ is the default tolerance.

\subsection{Alternative solver: GMRES}

As an alternative to CBICG, the code provides a GMRES (Generalized Minimal Residual) solver (Saad \& Schultz 1986) via Krylov.jl, selectable with \texttt{solver=:gmres}.  GMRES requires only the forward operator $\mathbf{L}\,\bm{v}$ (no adjoint) and minimises the 2-norm of the residual over a Krylov subspace.

Given the system $\mathbf{L}\,\bm{x} = \bm{b}$ with initial guess $\bm{x}_0$ and initial residual $\bm{r}_0 = \bm{b} - \mathbf{L}\,\bm{x}_0$, GMRES constructs at iteration $k$ the Krylov subspace
\begin{equation}
...
\end{equation}
and finds the approximate solution $\bm{x}_k = \bm{x}_0 + \bm{z}_k$ where $\bm{z}_k \in \mathcal{K}_k$ minimises the residual norm:
\begin{equation}
  \bm{z}_k = \arg\min_{\bm{z} \in \mathcal{K}_k} \|\bm{b} - \mathbf{L}\,(\bm{x}_0 + \bm{z})\|_2.
  \label{eq:gmres-minimisation}
\end{equation}

The algorithm proceeds as follows:
\begin{enumerate}
  \item \textbf{Arnoldi process.}  Build an orthonormal basis $\{\bm{v}_1, \ldots, \bm{v}_k\}$ of $\mathcal{K}_k$ via modified Gram--Schmidt.  At each step $j$, one matrix-vector product $\bm{w} = \mathbf{L}\,\bm{v}_j$ is computed, followed by orthogonalisation against $\bm{v}_1, \ldots, \bm{v}_j$.  This produces the upper Hessenberg matrix $\bar{\mathbf{H}}_k \in \mathbb{C}^{(k+1) \times k}$ such that $\mathbf{L}\,\mathbf{V}_k = \mathbf{V}_{k+1}\,\bar{\mathbf{H}}_k$.

  \item \textbf{Least-squares solve.}  Minimise $\|\beta\,\bm{e}_1 - \bar{\mathbf{H}}_k\,\bm{y}\|_2$ where $\beta = \|\bm{r}_0\|_2$ and $\bm{e}_1$ is the first unit vector.  This small $(k{+}1) \times k$ problem is solved efficiently via Givens rotations.

  \item \textbf{Solution update.}  $\bm{x}_k = \bm{x}_0 + \mathbf{V}_k\,\bm{y}_k$.
\end{enumerate}

Key properties:
\begin{itemize}
  \item \textbf{1 operator application per iteration} (no adjoint needed), versus 2 for CBICG.
  \item \textbf{Monotonically decreasing residual}: $\|\bm{r}_k\|_2 \leq \|\bm{r}_{k-1}\|_2$ by construction, unlike BiCG variants which can exhibit erratic convergence.
  \item \textbf{Memory}: stores $k$ basis vectors of length $n$, i.e., $O(kn)$ total.  For the system sizes in this code ($n \sim 10^4$, $k \sim 30$), this amounts to ${\sim}\SI{14}{\mega\byte}$.
  \item The implementation uses restarted GMRES($m$) with $m = 30$ (Krylov.jl \texttt{memory} parameter), restarting the Arnoldi process if $k$ exceeds $m$.
\end{itemize}

\textbf{Performance comparison with CBICG for this problem class.}
GMRES requires approximately twice as many iterations as CBICG to reach the same residual tolerance, because the CBICG adjoint operator (computed via S-parity, \eqref{eq:adjoint-symmetry}) provides additional directional information that accelerates convergence.  The total number of $\mathbf{A}$-operator applications is therefore similar (CBICG: $k$ iterations $\times$ 2; GMRES: $\sim 2k$ iterations $\times$ 1), and the wall-clock performance is comparable.  The default solver remains CBICG, consistent with the Fortran MSTM reference implementation.
...
\begin{tabular}{@{} rlll @{}}
  ...
  5 & CBICG solve & $(\mathbf{I} - \mathbf{T}\mathbf{A})\bm{x} = \mathbf{T}\bm{p}_\mathrm{inc}$ & \texttt{TMatrixSolver.jl} \\
...
\begin{tabular}{@{} p{4.2cm} p{2.8cm} p{4.2cm} p{3.5cm} @{}}
  ...
  CBICG solver & \eqref{eq:global-system}, \eqref{eq:adjoint-symmetry}--\eqref{eq:convergence} & \texttt{\_solve\_bicg} & \texttt{TMatrixSolver.jl} \\
  GMRES solver (optional) & \eqref{eq:global-system}, \eqref{eq:krylov-subspace}--\eqref{eq:gmres-minimisation} & \texttt{gmres} (Krylov.jl) & \texttt{TMatrixSolver.jl} \\
