...
**Last updated:** 2026-03-26 (corresponds to MSTMforCAS.jl v0.3.0)
...
2. [Single-Sphere Scattering: Lorenz–Mie Theory](#2-single-sphere-scattering-lorenzmie-theory)
3. [Multi-Sphere Interaction Equation](#3-multi-sphere-interaction-equation)
4. [Iterative Solution: Complex BiConjugate Gradient](#4-iterative-solution-complex-biconjugate-gradient)
5. [Output Quantities](#5-output-quantities)
6. [Computational Flow Summary](#6-computational-flow-summary)
...
```math
k = \frac{2\pi \, n_\mathrm{med}}{\lambda_0}. \qquad \textrm{(1)}
```
...
```math
a_\mathrm{eff} = \left(\sum_{i=1}^{N_S} a_i^3\right)^{\!1/3}, \qquad x_\mathrm{eff} = k\,a_\mathrm{eff}. \qquad \textrm{(2)}
```
...
```math
\mathbf{E}_\mathrm{sca}^{(i)}(\mathbf{r}) = \sum_{n=1}^{N_i} \sum_{m=-n}^{n} \left[ a_{mn}^{(i)}\,\mathbf{M}_{mn}^{(3)}(\mathbf{r} - \mathbf{r}_i) + b_{mn}^{(i)}\,\mathbf{N}_{mn}^{(3)}(\mathbf{r} - \mathbf{r}_i) \right] \qquad \textrm{(3)}
```
...
```math
a_n = \frac{\left[\dfrac{D_n(m_\mathrm{rel}\,x)}{m_\mathrm{rel}} + \dfrac{n}{x}\right] \psi_n(x) - \psi_{n-1}(x)}{\left[\dfrac{D_n(m_\mathrm{rel}\,x)}{m_\mathrm{rel}} + \dfrac{n}{x}\right] \xi_n(x) - \xi_{n-1}(x)} \qquad \textrm{(5)}
```

```math
b_n = \frac{\left[m_\mathrm{rel}\,D_n(m_\mathrm{rel}\,x) + \dfrac{n}{x}\right] \psi_n(x) - \psi_{n-1}(x)}{\left[m_\mathrm{rel}\,D_n(m_\mathrm{rel}\,x) + \dfrac{n}{x}\right] \xi_n(x) - \xi_{n-1}(x)} \qquad \textrm{(6)}
```
...
```math
D_{n-1}(y) = \frac{n}{y} - \frac{1}{D_n(y) + n/y}, \qquad \textrm{(7)}
```
...
```math
f_n(x) = \frac{2n - 1}{x}\,f_{n-1}(x) - f_{n-2}(x) \qquad \textrm{(8)}
```
...
```math
N_i = \mathrm{round}\!\left(x_i + 4\,x_i^{1/3}\right) + 5, \qquad \textrm{(9)}
```
...
```math
\mathbf{T}_n = -\frac{1}{2}\begin{pmatrix} a_n + b_n & a_n - b_n \\ a_n - b_n & a_n + b_n \end{pmatrix} \qquad \textrm{(10)}
```
...
```math
\mathbf{a}^{(i)} = \mathbf{T}_i\,\mathbf{p}_\mathrm{exc}^{(i)}. \qquad \textrm{(12)}
```
...
```math
(\mathbf{I} - \mathbf{T}\,\mathbf{A})\,\mathbf{x} = \mathbf{T}\,\mathbf{p}_\mathrm{inc} \qquad \textrm{(13)}
```
...
```math
C_{mn,kl} = \sum_{w=w_\mathrm{min}}^{n+l} c_w(m,n,k,l) \; h_w^{(1)}(d_{ij}) \; P_w^{m-k}(\cos\theta_{ij}) \qquad \textrm{(14)}
```
...
```math
H_{kl,mn}^{(p=1)} = e^{i(m-k)\phi_{ij}} \left(A + iB\right), \qquad H_{kl,mn}^{(p=2)} = e^{i(m-k)\phi_{ij}} \left(A - iB\right). \qquad \textrm{(15)}
```
...
```math
\mathbf{A}^\dagger = \left(\mathbf{S}\,\mathbf{A}\,\mathbf{S}\right)^* \qquad \textrm{(17)}
```
...
```math
\frac{\mathrm{Re}(\mathbf{r}^\top \mathbf{r})}{\|\mathbf{b}\|^2} < \varepsilon \qquad \textrm{(18)}
```
...
```math
p_{mn,p=1,q=1}^{(i)} = i^{\,n+1}\sqrt{\frac{2n+1}{2}}\;\delta_{m,+1}\;e^{iz_i}, \qquad p_{mn,p=2,q=1}^{(i)} = -i^{\,n+1}\sqrt{\frac{2n+1}{2}}\;\delta_{m,-1}\;e^{iz_i} \qquad \textrm{(19)}
```

for the first incident polarisation ($`q = 1`$), and

```math
p_{mn,p=1,q=2}^{(i)} = -i^{\,n+2}\sqrt{\frac{2n+1}{2}}\;\delta_{m,+1}\;e^{iz_i}, \qquad p_{mn,p=2,q=2}^{(i)} = -i^{\,n+2}\sqrt{\frac{2n+1}{2}}\;\delta_{m,-1}\;e^{iz_i} \qquad \textrm{(20)}
```
...
```math
\mathbf{a}_0 = \sum_{i=1}^{N_S} \mathbf{J}_{0i}\,\mathbf{a}^{(i)} \qquad \textrm{(21)}
```
...
```math
a_{mn}^{(\text{mode}\,1)} = a_{mn}^{(p=1)} + a_{mn}^{(p=2)}, \qquad a_{mn}^{(\text{mode}\,2)} = a_{mn}^{(p=1)} - a_{mn}^{(p=2)}. \qquad \textrm{(22)}
```
...
```math
\begin{pmatrix} E_\parallel^\mathrm{sca} \\ E_\perp^\mathrm{sca} \end{pmatrix} = \frac{e^{ikr}}{-ikr} \begin{pmatrix} S_2 & S_3 \\ S_4 & S_1 \end{pmatrix} \begin{pmatrix} E_\parallel^\mathrm{inc} \\ E_\perp^\mathrm{inc} \end{pmatrix} \qquad \textrm{(23)}
```
...
```math
S_j = -2 \sum_{n=1}^{N_\mathrm{max}} \sum_{m = \pm 1} \sum_{p = 1}^{2} F_j(n, m, p)\;\tau_{m,n,p}(\theta) \qquad \textrm{(24)}
```
...
```math
Q_\mathrm{ext} = -\frac{2}{x_\mathrm{eff}^2} \sum_{q=1}^{2} \mathrm{Re}\!\left(\mathbf{a}_q^\dagger \,\mathbf{p}_{\mathrm{inc},q}\right) \qquad \textrm{(25)}
```
...
```math
Q_\mathrm{sca} = \frac{1}{x_\mathrm{eff}^2} \sum_{q=1}^{2} \left(\|\mathbf{a}_{0,q}^{(\text{mode}\,1)}\|^2 + \|\mathbf{a}_{0,q}^{(\text{mode}\,2)}\|^2 \right) \qquad \textrm{(26)}
```
...
```math
Q_\mathrm{abs} = Q_\mathrm{ext} - Q_\mathrm{sca} \qquad \textrm{(27)}
```
...
```math
S_{11}^\mathrm{MI02} = \frac{S_2^\mathrm{BH83}}{-ik}, \quad S_{22}^\mathrm{MI02} = \frac{S_1^\mathrm{BH83}}{-ik}, \quad S_{12}^\mathrm{MI02} = \frac{S_3^\mathrm{BH83}}{ik}, \quad S_{21}^\mathrm{MI02} = \frac{S_4^\mathrm{BH83}}{ik} \qquad \textrm{(28)}
```
...
```math
S_s^\mathrm{fwd} = S_{11}^\mathrm{MI02} + i\,S_{12}^\mathrm{MI02} \qquad \textrm{(29)}
```

```math
S_p^\mathrm{fwd} = S_{22}^\mathrm{MI02} - i\,S_{21}^\mathrm{MI02} \qquad \textrm{(30)}
```

```math
S^\mathrm{bak} = \frac{-S_{11}^\mathrm{MI02,bwd} + S_{22}^\mathrm{MI02,bwd}}{\sqrt{2}} \qquad \textrm{(31)}
```
...
|---|---|---|---|
| Wavenumber and non-dimensionalisation | Eq. (1)–(2) | `compute_scattering`, `run_parameter_sweep` | [ParameterSweep.jl](../src/ParameterSweep.jl) |
| Lorenz–Mie coefficients $`a_n, b_n`$ | Eq. (5)–(9) | `mie_coefficients` | [MieCoefficients.jl](../src/MieCoefficients.jl) |
...
| Cross-section efficiencies | Eq. (25)–(27) | `compute_efficiencies` | [ScatteringAmplitude.jl](../src/ScatteringAmplitude.jl) |
| MI02 conversion | Eq. (28) | `bh83_to_mi02` | [ParameterSweep.jl](../src/ParameterSweep.jl) |
| CAS-v2 observables | Eq. (29)–(31) | `read_results_h5_for_cas.jl` | [scripts/read_results_h5_for_cas.jl](../scripts/read_results_h5_for_cas.jl) |
| Aggregate geometry I/O | — | `read_aggregate_file`, `read_aggregate_catalog` | [AggregateIO.jl](../src/AggregateIO.jl) |

---

## 8. References

1. Bohren, C.F. & Huffman, D.R. (1983). *Absorption and Scattering of Light by Small Particles*. Wiley. [BH83]
2. Mackowski, D.W. (1991). Analysis of radiative scattering for multiple sphere configurations. *Proc. R. Soc. Lond. A*, 433, 599–614.
3. Mackowski, D.W. (1994). Calculation of total cross sections of multiple-sphere clusters. *JOSA A*, 11(11), 2851–2861.
4. Mackowski, D.W. (1996). Calculation of the total scattering cross section and the total absorbed energy for the multiple sphere clusters. *JOSA A*, 13(11), 2266–2278.
5. Mackowski, D.W. (2014). A general superposition solution for electromagnetic scattering by multiple spherical domains of optically active media. *JQSRT*, 133, 264–270.
6. Mackowski, D.W. & Kolokolova, L. (2022). Application of the multiple sphere superposition solution to large-scale systems of spheres via an accelerated algorithm. *JQSRT*, 280, 108075.
